...
// اضافه کردن قابلیت حذف ویدیو با ID
router.delete("/videos/:id", async (req, res) => {
  try {
    const { id } = req.params;
    
    // فراخوانی تابع حذف از فایل db.js
    await deleteVideo(id); 
    
    res.json({ success: true, message: `ویدیو با آی‌دی ${id} پاک شد` });
  } catch (error) {
    console.error("Delete Error:", error);
    res.status(500).json({ error: "خطا در حذف ویدیو از دیتابیس" });
  }
});
